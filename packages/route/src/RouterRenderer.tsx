...
const RouterRenderer = () => {
  ...
  let routeComponent = (
    <RouteComponent component={firstItem.component} prepared={firstItem.prepared} routeData={firstItem.routeData} />
  );
  for (let ii = 1; ii < reversedItems.length; ii++) {
    const nextItem = reversedItems[ii];
    routeComponent = (
      <RouteComponent component={nextItem.component} prepared={nextItem.prepared} routeData={nextItem.routeData}>
        {routeComponent}
      </RouteComponent>
    );
  }
  ...
  return (
    <ErrorBoundary>
      <Suspense fallback={'Loading fallback...'}>
        ...
        {isPending ? <Pending>Loading pending...</Pending> : null}
        {routeComponent}
      </Suspense>
    </ErrorBoundary>
  );
